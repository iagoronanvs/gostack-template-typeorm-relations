...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (insufficientQuantity.length) {
      throw new AppError(
        `Product ${insufficientQuantity[0].id} does not have enough inventory`,
      );
    }
    ...
    const listToSubtractQuantityProduct = products.map(product => ({
      id: product.id,
      quantity:
        findProducts.filter(p => p.id === product.id)[0].quantity -
        product.quantity,
    ...
    await this.productsRepository.updateQuantity(listToSubtractQuantityProduct);
